...
var supabase = (0, _supabaseJs.createClient)("https://dcqhpsfenxvhmnlpskaq.supabase.co", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRjcWhwc2Zlbnh2aG1ubHBza2FxIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTY5MDM5NjQ1OCwiZXhwIjoyMDA1OTcyNDU4fQ.mhEDGNoN_iZIs6lmczAsVLYq_BmfNcl6xV1-yagGAFA");
var userForm = document.getElementById("userForm");
var networthOutput = document.getElementById("networth");
userForm.addEventListener("submit", /*#__PURE__*/function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(event) {
    var name, assets, liabilities, volatility, riskFreeRate, tenure, networth, callOptionPremium, formData, _yield$supabase$from$, data, error, message;
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          ...
          tenure = parseFloat(document.getElementById("tenure").value); // Calculate net worth
          networth = assets - liabilities;
          callOptionPremium = blackScholesCallOptionPremium(assets, liabilities, volatility, riskFreeRate, tenure);
          formData = {
            name: name,
            assets: assets,
            ...
            tenure: tenure,
            networth: networth,
            callOptionPremium: callOptionPremium // Add the call option premium to the formData object
          };
          _context.prev = 10;
          _context.next = 13;
          return supabase.from("users").insert([formData]);
        case 13:
          ...
          if (error) {
            ...
            message = "Hi ".concat(name, ", For the option with asset value of $").concat(assets, " , exercise price of $").concat(liabilities, ", riskfree rate of $").concat(riskFreeRate, ", volatilit of $").concat(volatility, " and tenure of $").concat(tenure, "  The estimated call option premium is $").concat(callOptionPremium, ".");
        ...
        case 19:
          ...
          console.error("Error occurred while processing the form:", _context.t0);
          alert("Error occurred while processing the form.");
        case 23:
        case "end":
...
function blackScholesCallOptionPremium(S,
// Current stock price (assets)
X,
...
T // Tenure (in years)
  ...
  var d1 = (Math.log(S / X) + (r + 0.5 * Math.pow(sigma, 2)) * T) / (sigma * Math.sqrt(T));
  var d2 = d1 - sigma * Math.sqrt(T);
  var callOptionPremium = S * cumulativeNormalDistribution(d1) - X * Math.exp(-r * T) * cumulativeNormalDistribution(d2);
...
function cumulativeNormalDistribution(x) {
  ...
  var cnd = RSQRT2PI * Math.exp(-0.5 * x * x) * (k * (a1 + k * (a2 + k * (a3 + k * (a4 + k * a5)))));
  if (x > 0) {
    return 1.0 - cnd;
  } else {
    return cnd;
